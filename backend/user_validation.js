function(newDoc, oldDoc, userCtx, secObj) {
    ...
    if(typeof newDoc.name !== "string"){
        throw ({ forbidden: 'doc.name is not of type string' });
    }

    if(!newDoc.displayName) {
        throw ({ forbidden: 'doc.displayname is required' });
    }

    if(typeof newDoc.displayName !== "string"){
        throw ({ forbidden: 'doc.displayName is not of type string' });
    }

    if(!newDoc.image){
        throw({forbidden: 'doc.image is required'})
    }

    if(typeof newDoc.image !== "string"){
        throw ({ forbidden: 'doc.image is not of type string' });
    }
